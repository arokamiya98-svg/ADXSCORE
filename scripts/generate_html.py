...
def aggregate_to_weekly(records: list[dict], symbol: str) -> dict:
    """
    records: scores.json の全件
    return: {
      "XAUUSD": {
        "2025-W16": {"ws": "2025.04.14", "h1a": 28.5, "h4p20": 85.0, "h4p30": 40.0},
        ...
      }
    }
    """
...
def generate_html(records: list[dict]) -> str:
    ...
    raw_js = {}
    for sym in SYMBOLS:
        raw_js[sym] = raw_by_sym.get(sym, {})

    raw_json     = json.dumps(raw_js,      ensure_ascii=False)
...
const ALL_SYMS     = {symbols_json};
const RECENT_5     = {recent_json};
const UPDATED_AT   = "{now_jst}";
</script>
        ...
        {{RECENT_5.map(r=>{{
          ...
          return (
            <div key={{r.date}} style={{{{
              ...
              <div style={{{{fontSize:9,color:"#5a8aaa",marginBottom:4}}}}>{label}({dow})</div>
              <div style={{{{fontSize:22,fontWeight:800,color:c.tx,lineHeight:1}}}}>{s!=null?Math.round(s):"—"}</div>
              <div style={{{{fontSize:9,fontWeight:700,color:c.tx,marginTop:2}}}}>{{scoreLabel(s)}}</div>
  ...
  return (
    <div style={{{{background:"#060b10",minHeight:"100vh"}}}}>
      ...
      <div style={{{{background:"linear-gradient(135deg,#0a1828,#060b10)",borderBottom:"1px solid #122030",padding:"12px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}}}>
        <div>
          <div style={{{{fontSize:9,color:"#2a4a60",letterSpacing:2,marginBottom:2}}}}>
            ADX Score = √(H1norm × H4_20) × 0.85 × (1 + H4_30×0.5)
          </div>
      ...
      <div style={{{{minHeight:36,padding:"5px 16px",display:"flex",alignItems:"center"}}}}>
        {{hov&&hovR ? (
          <div style={{{{display:"flex",gap:10,background:"#0a1520",border:"1px solid #122030",borderRadius:6,padding:"5px 14px",fontSize:10,flexWrap:"wrap",alignItems:"center"}}}}>
            ...
            <span style={{{{color:"#3a5a70"}}}}>│</span>
            <span>H1avg: <b style={{{{color:"#aaccff"}}}}>{{hovR.h1a?.toFixed(2)??"—"}}</b></span>
            <span>H4≥20: <b style={{{{color:"#ffcc44"}}}}>{{hovR.h4p20?.toFixed(1)??"—"}}%</b></span>
            <span>H4≥30: <b style={{{{color:"#ff88cc"}}}}>{{hovR.h4p30?.toFixed(1)??"—"}}%</b>
      ...
      <div style={{{{padding:"0 16px 32px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:8,maxWidth:1100}}}}>
        {{[
          {{t:"📊 スコア設計",b:"√(H1norm × H4_20) × 0.85 がベース。H4_30はボーナス乗数（×1.0〜1.5）。H4_30=0でも死なない、H4_30が高いと明確に加点。"}},
          {{t:"🎯 点数の目安",b:"80以上🔥爆発 / 65以上→超強 / 50以上★トレード候補 / 38以上→良い / 27以上→OK / 18以上→様子見 / 18未満→見送り"}},
...
def main():
    ...
    if not os.path.exists(DATA_PATH):
        print("[WARN] scores.json が存在しません。空のHTMLを生成します。")
        records = []
    else:
